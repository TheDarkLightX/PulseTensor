...
contract PulseTensorCoreEchidna {
    ...
    function echidna_stake_and_native_liabilities_conserved() external view returns (bool) {
        ...
        uint256 recordedLiabilities = totalStake + core.challengeRewardOf(netuid, address(actorA))
            + core.challengeRewardOf(netuid, address(actorB)) + core.subnetEmissionPool(netuid);
        return totalStake == sumStake && address(core).balance == recordedLiabilities;
    }

    function echidna_validator_count_exact_and_bounded() external view returns (bool) {
        (, uint16 maxValidators,,,,,) = core.subnets(netuid);
        uint16 expectedCount = core.isValidator(netuid, address(actorA)) ? 1 : 0;
        if (core.isValidator(netuid, address(actorB))) {
            expectedCount += 1;
        }
        uint16 actualCount = core.validatorCount(netuid);
        return actualCount == expectedCount && actualCount <= maxValidators;
    }
