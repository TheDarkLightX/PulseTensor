  ...
  docker run --rm \
    ...
    "${ECHIDNA_IMAGE}" \
      bash -lc '
        ...
        cd /tmp/work
        forge config --json | python3 -c '\''import json,sys; c=json.load(sys.stdin); print("echidna_compile_profile=" + json.dumps({k:c[k] for k in ("solc","optimizer","optimizer_runs","via_ir","evm_version")}, sort_keys=True))'\''
